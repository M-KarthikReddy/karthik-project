...
import com.alacriti.paymentGateway.payment.gateway.api.merchant.common.Currency;
import com.alacriti.paymentGateway.payment.gateway.api.merchant.common.Status;
...
public class PaymentService {
	...
	public List<MerchantTransactions> returnTransactionByMerchantId(String mID) {
		
		List<PaymentEntity> listOfTrans = paymentRepository.findByMerchantId(mID);
		
		List<MerchantTransactions> response = new ArrayList<>();
		
		
		for(PaymentEntity entity: listOfTrans) {
			
			MerchantTransactions transactions = new MerchantTransactions();
			
			
			transactions.setTransactionId(entity.gettransactionId());
			transactions.setStatus(entity.getStatus());
			transactions.setOrderId(entity.getOrderId());
